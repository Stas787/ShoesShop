...
namespace ShoesShop.Repository
{
    public class ShoesRepository : IAllShoes
    {
        ...
        public IEnumerable<Shoes> GetFavoriteShoes => DataBaseContent.Shoes.Where(choose => choose.IsFavorite).Include(data => data.Category);

        public Shoes GetObjectShoes(int shoesID) => DataBaseContent.Shoes.FirstOrDefault(identifire => identifire.Id == shoesID); // choose one element or default variant
